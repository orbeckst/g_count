 ...
 * \date   2002
 * $Id: g_zcoord.c,v 1.17 2004/03/17 22:04:31 oliver Exp $
 */
...
static char *SRCID_g_zoord_c = "$Id: g_zcoord.c,v 1.17 2004/03/17 22:04:31 oliver Exp $";
...
#include "princ.h"
#include "names.h"
#ifdef NEEDS_GENERAL_H
/* does not exist in Gromacs 3.2 and higher (and is probably not needed anyway 
   but I havent checked that yet) 
*/  
#include "general.h" 
#endif
#include "matio.h"
...
{
  static char *desc[] = {
    "g_zcoord can output the z coordinates of atoms or the center of mass of ",
    "molecules as function of time which are located in a cylindrical region."
    ...
    "z coordinates for each entry in the index file.[PAR]",

    "For quicker visualisation the [TT]-mat[tt] option is useful. It creates ",
  ...
  t_cavity   cavity = {   /* define volume to count mols in */
    {0, 0, 1},            /* axis */
    {0, 0, 0},            /* cpoint  [PERSONAL DEFAULT] */
    1000,                  /* radius */
    -1000,                /* z1 < z2 */
    1000,
    -1                    /* volume - calculate later */
  };
  ...
  t_pargs pa[] = {
    { "-m", FALSE, etBOOL, {&bMolecular},
      "index contains molecule numbers instead of atoms"},
...
//! explanation for this guy - MB doxygen example
#define NFILE asize(fnm) 

//! explanation for another guy - MB doxygen example
#define NDFILES 2
  ...
  static char *bugs[] = { 
    ...
    "Feedback to <oliver@biop.ox.ac.uk>"
  };
  ...
  if (bMolecular)
    msg("Interpreting indexfile entries as molecules. "
  ...
  do {
	...
	for (j = 0; j < DIM; j++) {
	  ...
	  if (xcm[j] > box[j][j]) xcm[j]-=box[j][j];
        ...
        if (bXPM) {
          if ( bInCavity (xcm, &cavity) ) {  // only look at cylinder
            if ( (xcm[ZZ] >= minzval) && (xcm[ZZ] <= maxzval))
              zbins[k_frame][(int)floorl((xcm[ZZ] - minzval) / spacing)]=1.0;
          }
        }
	dump_data (dfiles,  "\t%10g", xcm[ZZ]);
      }
	 ...
	 of atoms */
      for(i=0; i<gnx; i++) {
        if (bXPM) {
          if ( bInCavity (x[index[i]], &cavity) ) {  // only look at cylinder
            if ( (x[index[i]][2] >= minzval) && (x[index[i]][2] <= maxzval))
              zbins[k_frame][(int)floorl((x[index[i]][2] - minzval) / spacing)]=1.0;
          }
        }
	dump_data (dfiles,"\t%10g", x[index[i]][2]);
      }
